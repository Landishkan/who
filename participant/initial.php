...
try {
    ...
    // Получаем все активные начальные тесты
    $tests = $pdo->query("
        SELECT 
            t.id as test_id,
            s.district,
...
// Получение device hash для защиты от повторных прохождений
...
if ($test_id) {
    try {
        ...
        if ($stmt->fetch()) {
            ...
            // Получаем вопросы для начального теста
            $stmt = $pdo->prepare("
                SELECT id, text, theme 
                FROM questions 
                WHERE test_type = 'initial' 
                ORDER BY theme, id
            ");
            $stmt->execute();
...
<head>
    ...
    <title>Начальный тест | КиберКвестор</title>
    ...
    <style>
        ...
        .test-card:hover {
            border-color: rgba(75, 205, 235, 0.8);
            box-shadow: 0 0 30px rgba(75, 205, 235, 0.3);
        ...
        .answer-btn:hover {
            background: rgba(75, 205, 235, 0.3);
            border-color: #4bcde7;
        }
    ...
    <div class="container">
        <header class="main-header">
            <div class="logo-pulse">
                <h1>КиберКвестор</h1>
                <p class="tagline">Начальный тест</p>
            </div>
                ...
                <div class="test-form">
                    <div class="already-passed">
                        ...
                        <p style="color: rgba(255,255,255,0.7); font-family: 'Roboto', sans-serif;">
                            Каждый участник может пройти тест только один раз.
                        </p>
                        <a href="initial.php" class="back-link">
        ...
        <footer class="cyber-footer">
            <div class="footer-logo">КиберКвестор — Начальный тест</div>
    ...
    <script>
        ...
        document.querySelectorAll('.answer-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                // Убираем выделение со всех кнопок этого вопроса
                ...
                // Сохраняем ответ в скрытое поле
