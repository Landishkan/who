...
async function showResults() {
    ...
    if (correctAnswers === totalQuestions) {
        message = `🛡️ Отлично! ${totalQuestions}/${totalQuestions}! Ты отлично разбираешься в защите от фишинга!`;
    ...
    if (user) {
        try {
            ...
            const success = await updateUserProgress(user.uid, 'lecture_5', correctAnswers);
            if (success) {
