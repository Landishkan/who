...
const checkResultsBtn = document.getElementById('check-results-btn');
 ...
 async function showResults() {
    ...
    if (correctAnswers === totalQuestions) {
        message = `🎉 Супер! Ты ответил правильно на все ${totalQuestions} вопросов! Ты мастер сетевого этикета!`;
    } else if (correctAnswers >= totalQuestions * 0.7) {
        message = `👍 Хороший результат! ${correctAnswers} из ${totalQuestions} правильных ответов. Почти идеально!`;
    } else {
        message = `🤔 ${correctAnswers} из ${totalQuestions}. Попробуй перечитать лекцию — эти знания помогут тебе в онлайн-общении!`;
    }
    ...
    if (user) {
        try {
            ...
            const success = await updateUserProgress(user.uid, 'lecture_2', correctAnswers);
            if (success) {
